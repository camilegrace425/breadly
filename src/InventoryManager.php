...
class InventoryManager {
    ...
    public function getRecallHistoryByDate($date_start, $date_end) {
        try {
            $sql = "SELECT sa.adjustment_id, sa.timestamp, u.username, sa.item_type, sa.item_id,
                        COALESCE(p.name, i.name) AS item_name, sa.adjustment_qty, sa.reason,
                        CASE WHEN sa.item_type = 'product' AND sa.adjustment_qty < 0 
                             THEN sa.adjustment_qty * p.price ELSE 0 END AS removed_value
                    FROM stock_adjustments sa
                    LEFT JOIN users u ON sa.user_id = u.user_id
                    LEFT JOIN products p ON sa.item_id = p.product_id AND sa.item_type = 'product'
                    LEFT JOIN ingredients i ON sa.item_id = i.ingredient_id AND sa.item_type = 'ingredient'
                    WHERE sa.reason LIKE '%recall%' AND sa.adjustment_qty < 0 
    ...
    public function undoRecall($adjustment_id, $user_id) {
        try {
            ...
            if (!$original) throw new Exception("Recall record not found.");
            if ($original['adjustment_qty'] >= 0) throw new Exception("Cannot undo: This record does not appear to be a stock removal.");
            ...
            $stmtLog->execute([$original['item_id'], $original['item_type'], $user_id, $qty_to_add_back, $new_reason]);
    ...
    public function getIngredientBatches($ingredient_id) {
        try {
            $sql = "SELECT * FROM ingredient_batches WHERE ingredient_id = ? AND quantity > 0 
    ...
    public function createBatch($ingredient_id, $user_id, $qty, $expiration_date, $reason) {
        try {
            ...
            $this->conn->query("CALL IngredientCheckLowStock()");
    ...
    public function updateBatchQuantity($batch_id, $user_id, $new_qty, $reason) {
        try {
            ...
            $diff = floatval($new_qty) - floatval($batch['quantity']);
            if ($diff == 0) {
                $this->conn->rollBack();
            ...
            $stmt2->execute([$new_qty, $batch_id]);
